...
export function segmentFoot(rect, ppm, wantDebug = false) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const dark = 1 - Y[i] / ref;
      const sc = Math.max(dark / CONFIG.SEG_DARK_T, chroma(i) / CONFIG.SEG_CHROMA_T);
  ...
  if (!big.length) return { mask: null, w, h, ppm, heelSide: null, stat: null, debug };
  const best = big[0];
  ...
  return { mask: out, w, h, ppm, heelSide, stat: best, contacts: { top, bottom }, touchesSides, debug };
}
